...
document.addEventListener('DOMContentLoaded', () => {
    // === Helper Function for Asset Paths (from previous conversation) ===
    ...
    // === Global Cart Management (can be in a separate module/file for larger apps) ===
    let cart = JSON.parse(localStorage.getItem('audiophileCart')) || [];
    ...
    if (window.location.pathname.includes('checkout.html')) {
        ...
        const renderCartSummary = () => {
            ...
            cart.forEach(item => {
                ...
                itemDiv.innerHTML = `
                        <img src="${correctAssetPath(item.image)}" alt="${item.name}">
